...
//
// This example shows how to compose these Steps:
...
func main() {
	...
	// Step 1: Initialize embedding model (using local BGE)
	embedder, err := embedding.WithBEG("bge-small-zh-v1.5", "")
	if err != nil {
		log.Fatalf("Failed to initialize embedder: %v", err)
	}

	// Step 2: Initialize LLM (using Mock for demonstration)
	llm := initMockLLM()

	// Step 3: Create Native RAG Searcher with composed Steps
	adapter := &govectorAdapter{collection: initVectorStore()}
	searcher := native.New(
		native.WithEmbedding(embedder),
		native.WithVectorStore(adapter),
		native.WithGenerator(service.NewGenerator(llm)), // Use service wrapper that implements retrieval.Generator
		native.WithTopK(10),
	)

	// Step 4: Execute query through the pipeline
	query := "GoRAG 支持哪些高级检索策略？"
...
// initVectorStore initializes a local govector store for demonstration
...
func (a *govectorAdapter) Add(ctx context.Context, vector *entity.Vector) error {
	// TODO: Implement based on govector API
	return nil
}
...
// initMockLLM creates a mock LLM for demonstration purposes
func initMockLLM() *MockLLM {
	return &MockLLM{}
}
...
func (m *MockLLM) Chat(ctx context.Context, messages []core.Message, opts ...core.Option) (*core.Response, error) {
	return &core.Response{
		Content: "这是一个示例答案。在实际应用中，LLM 会基于检索到的内容生成准确的回答。",
	}, nil
}
