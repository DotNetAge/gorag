...
- **🔄 连接池** - 向量存储连接池，支持连接复用和限流
- **🧠 查询缓存** - 基于LRU策略的内存缓存，支持TTL过期
- **⏰ 懒加载** - 大文档延迟加载与内存管理，支持自动卸载
- **⚡ 批处理优化** - 并发处理embedding生成，支持速率限制和指数退避重试
- **🔒 熔断器** - 防止级联故障，支持关闭、打开、半开三种状态
- **📉 优雅降级** - 多级降级策略，根据错误计数和延迟自动调整服务级别
...
- **embedding** - 嵌入模型提供器（OpenAI、Ollama、Cohere、Voyage）
