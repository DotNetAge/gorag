...
// =============================================================================
// 集成测试：LLM 索引器（分块 + 实体提取 + 标签 + 双写 vectorDB + graphDB）
//
// 环境变量:
//
...
func TestLLMIndexer_AddContent(t *testing.T) {
...
func TestLLMIndexer_AddFile(t *testing.T) {
	if os.Getenv("GORAG_API_KEY") == "" {
		t.Skip("跳过: 未设置 GORAG_API_KEY 环境变量")
	}
...
func TestLLMIndexer_Search(t *testing.T) {
	if os.Getenv("GORAG_API_KEY") == "" {
		t.Skip("跳过: 未设置 GORAG_API_KEY 环境变量")
	}
...
func TestLLMIndexer_SliceAndMerge(t *testing.T) {
	if os.Getenv("GORAG_API_KEY") == "" {
		t.Skip("跳过: 未设置 GORAG_API_KEY 环境变量")
	}
