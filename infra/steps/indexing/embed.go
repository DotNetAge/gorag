...
func Batch(embedder embedding.Provider, metrics abstraction.Metrics) pipeline.Step[*indexing.State] {
	return &batch{
		embedder: embedder,
		metrics:  metrics,
	}
...
func (s *batch) Execute(ctx context.Context, state *indexing.State) error {
	if s.embedder == nil {
