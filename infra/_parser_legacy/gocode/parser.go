...
func (p *Parser) parseLineByLine(ctx context.Context, r io.Reader, callback func(model.Chunk) error) error {
...
func (p *Parser) chunkText(ctx context.Context, text string, callback func(model.Chunk) error) error {
	if len(text) == 0 {
		return nil
	}
...
func (p *Parser) createChunk(content string, position int, elementType string) model.Chunk {
	metadata := map[string]string{
		"type":         "gocode",
