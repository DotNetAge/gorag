...
DEBUG: Debugging information`)
	...
	err := parser.ParseWithCallback(ctx, bytes.NewReader(logContent), func(chunk model.Chunk) error {
		chunkCount++
	...
	for _, file := range files {
		...
		t.Run(file.Name(), func(t *testing.T) {
			...
			for i, chunk := range chunks {
				...
				assert.Contains(t, chunk.Metadata["type"], "log", "Chunk %d should have type 'log'", i)
			}
		})
	}
}
